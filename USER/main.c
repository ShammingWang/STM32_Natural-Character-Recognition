...
//	esp8266_tcp_post(); //发送识别请求
	
 	
RESTART:
	ESP8266_RESTART:
	while(esp8266_init() || esp8266_tcp_init()) {
		printf("正在自动重启ESP8266...\r\n");
		Show_Str(30,150,200,16,(u8*)"正在自动重启ESP8266...",16,0);
	...
	LCD_Clear(WHITE);
	Show_Str(30,10,200,16,(u8*)"战舰STM32F103ZET6",16,0);				    	 
	Show_Str(30,30,200,16,(u8*)"基于STM32的手写输入识别系统",16,0);	//
	Show_Str(30,50,200,16,(u8*)"计科201/2",16,0);				    	 
	Show_Str(30,70,200,16,(u8*)"王尚民 姚远 周禧宇",16,0);			    
	Show_Str(30,90,200,16,(u8*)"识别结果:",16,0);
	
	LCD_DrawRectangle(19,114,lcddev.width-20,lcddev.height-5);
	//LCD_DrawRectangle(49,400-5,lcddev.width-50,lcddev.height-5);
	POINT_COLOR=BLUE;
	//Show_Str(96,207,200,16,(u8*)"手写区",16,0);
	//tcnt=100;
	tcnt=100;
	while(1)
	{
		...
		//Show_Str(88,207,200,16,(u8*)"全部识别",16,0);
 		tp_dev.scan(0);//扫描
		...
		}else //按键松开了 
		{
			...
			lastpos[0]=0XFFFF; //初始化下一次的上一个点
			tcnt++;
			delay_ms(10); //松开一次延时10ms
			//延时识别
			i++;
			//printf("tcnt: %d\r\n", tcnt);
			if(tcnt==40) //松开了400ms 认为此次输入结束
			{
				if(pcnt)//有 有效的输入
				{
					for(i=0;i<10;i++) res[i]=0;
					p = esp8266_tcp_post(); //发送识别请求
					strcpy((char*)res, p); //写入结果数组中
